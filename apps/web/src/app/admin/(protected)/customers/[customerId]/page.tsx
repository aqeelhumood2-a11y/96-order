...
export default async function CustomerDetailPage({ params }: CustomerDetailPageProps) {
  ...
  const dict = getDictionary(locale).admin.customerDetail;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-md border border-brand-100 p-4">
          <p className="text-xs uppercase tracking-wide text-foreground/65">{dict.totalSpent}</p>
          <p className="text-xl font-semibold text-brand-950">{formatMoney(customer.totalSpent)}</p>
        </div>
        <div className="rounded-md border border-brand-100 p-4">
          <p className="text-xs uppercase tracking-wide text-foreground/65">{dict.firstOrder}</p>
          <p className="text-xl font-semibold text-brand-950">{formatDate(customer.firstOrderAt)}</p>
        </div>
        <div className="rounded-md border border-brand-100 p-4">
          <p className="text-xs uppercase tracking-wide text-foreground/65">{dict.lastOrder}</p>
      ...
      <div className="flex flex-col gap-2 rounded-md border border-brand-100 p-4">
        <h2 className="text-sm font-semibold text-brand-950">{dict.contactInfo.title}</h2>
        <dl className="flex flex-col gap-1.5 text-sm">
          <div className="flex justify-between gap-4">
            <dt className="text-foreground/69">{dict.contactInfo.email}</dt>
            <dd className="text-foreground">{customer.email}</dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt className="text-foreground/69">{dict.contactInfo.mobile}</dt>
            <dd className="text-foreground">{customer.mobile}</dd>
          </div>
          {customer.companyName && (
            <div className="flex justify-between gap-4">
              <dt className="text-foreground/69">{dict.contactInfo.company}</dt>
              <dd className="text-foreground">{customer.companyName}</dd>
            </div>
          )}
          <div className="flex justify-between gap-4">
            <dt className="text-foreground/69">{dict.contactInfo.accountType}</dt>
            <dd className="text-foreground">{customer.kind === "guest" ? dict.contactInfo.guest : dict.contactInfo.registered}</dd>
          </div>
        </dl>
      </div>

      <div className="flex flex-col gap-2">
        <h2 className="text-sm font-semibold text-brand-950">{dict.orderHistory}</h2>
