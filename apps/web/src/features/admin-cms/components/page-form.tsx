...
export function CmsPageForm({ existing, locale = DEFAULT_LOCALE }: { existing?: CmsPage; locale?: Locale }) {
  const router = useRouter();
  const dict = getDictionary(locale).admin.cmsPageForm;
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex max-w-2xl flex-col gap-3">
      ...
      <Button type="submit" size="sm" disabled={isSubmitting} className="w-fit">
        {isSubmitting ? dict.saving : existing ? dict.saveChanges : dict.createPage}
      </Button>
    </form>
  );
}
