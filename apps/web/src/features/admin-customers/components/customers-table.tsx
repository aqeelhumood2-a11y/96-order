...
export function CustomersTable({ customers }: { customers: Customer[] }) {
  if (customers.length === 0) {
    return <p className="text-sm text-foreground/60">No customers match these filters.</p>;
  }
          ...
          {customers.map((customer) => (
            <tr key={customer.id} className="border-b border-brand-50 last:border-0 hover:bg-brand-50/30">
              <td className="px-4 py-3">
                ...
                {customer.companyName && <div className="text-xs text-foreground/50">{customer.companyName}</div>}
              </td>
              <td className="px-4 py-3">
                <div className="flex flex-col">
                  <span className="text-foreground">{customer.email}</span>
                  <span className="text-xs text-foreground/50">{customer.mobile}</span>
                </div>
              </td>
