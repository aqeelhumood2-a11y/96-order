...
export function CartSummary({
  ...
  return (
    <div className="rounded-lg border border-brand-100 bg-brand-50/40 p-6">
      <h2 className="text-lg font-semibold text-brand-950">{dict.orderSummary}</h2>
      <dl className="mt-4 space-y-2 text-sm">
        <div className="flex justify-between">
          <dt className="text-foreground/70">{dict.subtotal}</dt>
        ...
        {priced.appliedDiscounts.map((discount) => (
          <div key={`${discount.source}-${discount.id}`} className="flex justify-between text-accent-700">
            ...
            <dd className="font-medium">{discount.freeShipping ? dict.freeDelivery : `-${formatMoney(discount.amount)}`}</dd>
          </div>
        ))}
        <div className="flex justify-between">
          <dt className="text-foreground/70">{dict.delivery}</dt>
        ...
        <div className="flex justify-between border-t border-brand-200 pt-2 text-base">
          <dt className="font-semibold text-brand-950">{dict.total}</dt>
      ...
      {editableCoupon && (
        <div className="mt-4 border-t border-brand-200 pt-4">
          <CouponForm appliedCoupon={appliedCoupon} locale={locale} />
        </div>
      )}
    </div>
  );
