...
describe("FirebaseProductImageStorage (emulator)", () => {
  ...
  it("getDownloadUrl() re-normalizes an already-saved Drive URL and routes it through the /api/drive-image proxy, without touching the bucket", async () => {
    const staleUrl = "https://drive.google.com/uc?export=view&id=abc123";
    await expect(storage.getDownloadUrl(staleUrl)).resolves.toBe("/api/drive-image/abc123");
  });
