...
| `GOOGLE_DRIVE_API_KEY` | Lets `/api/drive-image/[fileId]` (`app/api/drive-image/[fileId]/route.ts`) fetch a Drive-hosted product image through the official Drive API instead of the browser hotlinking `lh3.googleusercontent.com` directly — see `core/catalog/rules.ts#toDriveProxyUrl`'s doc comment. Optional upgrade path, not required for the default hotlink behavior to work. Get one from [Google Cloud Console](https://console.cloud.google.com/apis/credentials) → **Create credentials → API key**, after enabling the **Google Drive API** on that project (APIs & Services → Library → search "Google Drive API" → Enable). No OAuth or service account needed — a plain API key can read any file shared "Anyone with the link". Restrict the key to the Drive API only. | Falls back to the direct `lh3.googleusercontent.com` hotlink — this app's default, unproxied behavior (byte-for-byte the same conversion used by this project's other Google-Drive-backed site). |
| `NEXT_PUBLIC_GOOGLE_DRIVE_IMAGE_PROXY_ENABLED` | Set to `"true"` alongside `GOOGLE_DRIVE_API_KEY` so the admin product-image form's *live preview* (before saving) also uses the proxy — a client component can't read the server-only API key directly, so this is how it learns the proxy is available. | Live preview uses the direct hotlink — the saved/storefront image still benefits from `GOOGLE_DRIVE_API_KEY` alone via `getDownloadUrl`, this only affects the pre-save preview box. |
