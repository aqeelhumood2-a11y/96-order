...
| `GOOGLE_DRIVE_API_KEY` | Lets `/api/drive-image/[fileId]` (`app/api/drive-image/[fileId]/route.ts`) fetch a Drive-hosted product image through the official Drive API instead of the browser hitting `drive.google.com`'s direct-view URL directly — see `core/catalog/rules.ts#toDriveProxyUrl`'s doc comment. Optional upgrade path, not required for the default direct-link behavior to work. Get one from [Google Cloud Console](https://console.cloud.google.com/apis/credentials) → **Create credentials → API key**, after enabling the **Google Drive API** on that project (APIs & Services → Library → search "Google Drive API" → Enable). No OAuth or service account needed — a plain API key can read any file shared "Anyone with the link". Restrict the key to the Drive API only. | Falls back to the direct `drive.google.com/uc?export=view` link — this app's default, unproxied behavior. |
