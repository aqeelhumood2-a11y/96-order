...
`/api/drive-image/[fileId]` (`app/api/drive-image/[fileId]/route.ts`) proxies a Drive-hosted product image through the official Drive API instead of the browser hotlinking `lh3.googleusercontent.com` directly (which this app confirmed can 403 for a specific file despite fully correct "anyone with the link" sharing — see `core/catalog/rules.ts#toDriveProxyUrl`'s doc comment). It authenticates using the **same** `FIREBASE_ADMIN_CLIENT_EMAIL`/`FIREBASE_ADMIN_PRIVATE_KEY` credential listed above (see `infrastructure/google/drive-auth.ts`) — no separate API key or additional env var needed. Where that credential isn't set (local dev against the emulator), the route falls back to a redirect to the direct hotlink instead of erroring.
