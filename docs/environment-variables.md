...
| `GOOGLE_DRIVE_API_KEY` | Lets `/api/drive-image/[fileId]` (`app/api/drive-image/[fileId]/route.ts`) fetch a Drive-hosted product image through the official Drive API instead of the browser hotlinking `lh3.googleusercontent.com` directly — see `core/catalog/rules.ts#toDriveProxyUrl`'s doc comment for why that direct hotlink can return a genuine 403 from Google even for a correctly-shared file. Get one from [Google Cloud Console](https://console.cloud.google.com/apis/credentials) → **Create credentials → API key**, after enabling the **Google Drive API** on that project (APIs & Services → Library → search "Google Drive API" → Enable). No OAuth or service account needed — a plain API key can read any file shared "Anyone with the link". Restrict the key to the Drive API only. | Falls back to the direct `lh3.googleusercontent.com` hotlink — works for most files, but not guaranteed for every one (see above). |
